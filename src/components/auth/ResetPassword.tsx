...
export default function ResetPassword() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    setError(null)
    if (!tokenFromUrl) { setError('Kein gültiger Token vorhanden. Bitte fordere einen neuen Link an.'); return }
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        {done ? (
          <div style={{ background: 'white', padding: 24, borderRadius: 10, boxShadow: '0 8px 24px rgba(15,23,42,0.06)', textAlign: 'center' }}>
            <p style={{ color: '#22c55e', fontWeight: 600, fontSize: 18, marginBottom: 8 }}>Passwort geändert!</p>
          ...
          <form onSubmit={submit} style={{ background: 'white', padding: 24, borderRadius: 10, boxShadow: '0 8px 24px rgba(15,23,42,0.06)' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Passwort bestätigen" style={{ width: '100%', padding: 12, borderRadius: 8, border: '1px solid #e6e6e6' }} />
            </div>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <button type="submit" disabled={loading || !tokenFromUrl} style={{ padding: '10px 16px', background: '#2b6cb0', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer' }}>{loading ? 'Setze zurück...' : 'Passwort ändern'}</button>
              <button type="button" onClick={() => nav('/login')} style={{ padding: '10px 16px', borderRadius: 8, background: '#eef2ff', border: '1px solid #e6e6ff', cursor: 'pointer' }}>Zurück zum Login</button>
            </div>
          </form>
        )}
      </div>
